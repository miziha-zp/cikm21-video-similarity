...
class MultiModal(Model):
    def __init__(self, config, *args, **kwargs):
        ...
        self.bert = TFBertModel.from_pretrained(config.bert_dir, from_pt=False)
        self.bert_fc = tf.keras.layers.Dense(config.bert_fc_dim, activation=None)
    ...
    def call(self, inputs, **kwargs):
        ...
        bert_embedding2 = self.bert([inputs['input_ids'], inputs['mask']], output_hidden_states=True)
        ...
        return predictions, final_embedding, bert_embedding, bert_embedding2, vision_embedding
    ...
    def get_variables(self):
        if not self.all_variables:  # is None, not initialized
            ...
            self.normal_variables = self.nextvlad.trainable_variables + self.fusion.trainable_variables + \
                                    self.classifier.trainable_variables + self.bert_fc.trainable_variables
