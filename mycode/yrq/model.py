...
class MultiModal(Model):
    def __init__(self, config, *args, **kwargs):
        ...
        self.bert_fc = tf.keras.layers.Dense(1024)
        ...
        self.netvlad = NetVLAD(config.frame_embedding_size, config.max_frames,
                                config.vlad_cluster_size, config.vlad_hidden_size)
        self.netrvlad = NetRVLAD(config.frame_embedding_size, config.max_frames,
                                config.vlad_cluster_size, config.vlad_hidden_size)
        self.softdbow = SoftDBoW(config.frame_embedding_size, config.max_frames,
                                config.vlad_cluster_size, config.vlad_hidden_size)
        self.netfv = NetFV(config.frame_embedding_size, config.max_frames,
                                config.vlad_cluster_size, config.vlad_hidden_size)
    ...
    def call(self, inputs, **kwargs):
        ...
        bert_embedding = self.bert_fc(bert_embedding)
        frame_num = tf.reshape(inputs['num_frames'], [-1])
        vision_embedding_nextvlad = self.nextvlad([inputs['frames'], frame_num])
        vision_embedding_netvlad = self.netvlad(inputs['frames'])
        vision_embedding_netrvlad = self.netrvlad(inputs['frames'])
        vision_embedding_softdbow = self.softdbow(inputs['frames'])
        vision_embedding_netfv = self.netfv(inputs['frames'])
        
        final_embedding = self.fusion([vision_embedding_nextvlad, vision_embedding_netvlad, 
                                       vision_embedding_netrvlad, vision_embedding_softdbow,
                                       vision_embedding_netfv, bert_embedding])
        predictions = self.classifier(final_embedding)

        return predictions, final_embedding, bert_embedding, vision_embedding_nextvlad, vision_embedding_netrvlad, vision_embedding_softdbow, vision_embedding_netfv
    ...
    def get_variables(self):
        if not self.all_variables:  # is None, not initialized
            ...
            self.normal_variables = self.bert_fc.trainable_variables + \
                                    self.nextvlad.trainable_variables + \
                                    self.netvlad.trainable_variables + \
                                    self.netrvlad.trainable_variables + \
                                    self.softdbow.trainable_variables + \
                                    self.netfv.trainable_variables + \
